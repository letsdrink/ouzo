...
docker run --rm -e HOME=/var/www -e GIT_COMMITTER_NAME=docker -e GIT_COMMITTER_EMAIL=docker@docker \
  -e DB_HOST=$DOCKER_DB_NAME \
  --link $DOCKER_DB_NAME:$DOCKER_DB_NAME \
  ...
  './composer.phar install --no-ansi --no-interaction && \
    ...
    ./vendor/bin/phpunit --debug -d zend.enable_gc=0 --configuration phpunit.xml --exclude-group sqlite3 test'
